...
public class ChartRing extends VuiControl implements ICommonSupportChart, ISupportPanel {
    ...
    public void output(HtmlWriter html) {
        if (dataSet == null)
            return;
        title = dataSet.head().getString("title") + this.getClass().getSimpleName();
        ...
        html.println("</div>");
        html.println("<script>$(function(){buildRingChart(`%s`, '%s')})</script>", dataSet.json(), title);
    }
